...
template <typename ValueType> class optional_copy {
  public:
...
/*! \brief Same behavior as optional_copy, but with operator[] as
      shortcut for (*get())[]
   */
template <typename ContainerType> class optional_container : public optional_copy<ContainerType> {
  public:
    optional_container() {}
    ...
    typename ContainerType::value_type &operator[](typename ContainerType::size_type const &i) { return (*this->ptr_)[i]; }

    typename ContainerType::value_type const &operator[](typename ContainerType::size_type const &i) const { return (*this->ptr_)[i]; }
};
