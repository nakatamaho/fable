...
#define TBXX_ASSERT(condition)                                                                                                          \
    if (!(condition)) {                                                                                                                 \
        throw std::runtime_error(tbxx::error_utils::file_and_line_as_string(__FILE__, __LINE__) + ": ASSERT(" #condition ") failure."); \
    }

#define TBXX_UNREACHABLE_ERROR() std::runtime_error("Control flow passes through branch that should be unreachable: " + tbxx::error_utils::file_and_line_as_string(__FILE__, __LINE__))

#define TBXX_NOT_IMPLEMENTED() std::runtime_error("Not implemented: " + tbxx::error_utils::file_and_line_as_string(__FILE__, __LINE__))
