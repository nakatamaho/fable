...
def tokenize_expression_impl(
    ...
    if (allow_commas):
        tlist = []
        tokens.append(tk_seq(ssl=tokenizer.ssl,
                      i_code=tokenizer.i, value=tlist))
    else:
        tlist = tokens
    tapp = tlist.append
...
class vt_used(object):
    pass


class vt_scalar(object):
    pass


class vt_string(object):
    pass


class vt_array(object):
    pass


class vt_intrinsic(object):
    pass


class vt_external(object):
    pass


class vt_function(object):
    pass


class vt_subroutine(object):
    pass

# variable storage


class vs_fproc_name(object):
    pass


class vs_argument(object):
    pass


class vs_common(object):
    pass


class vs_save(object):
    pass


class vs_local(object):
    pass


class vs_parameter(object):
    pass
...
def extract_size_tokens(ssl, start):
    ...
    return \
        i_size, \
        [tokenization.tk_integer(
            ssl=ssl, i_code=start, value=code[start:i_size])]
        ...
        while True:
            ...
            if (c == "("):
                ...
                if (not allow_size or item_size_tokens is not None):
                    ssl.raise_syntax_error(i=i_code)
                i_code, item_size_tokens = extract_size_tokens(
                    ssl=ssl, start=i_code+1)
            else:
                ssl.raise_syntax_error(i=i_code)
...
class fproc_p_methods(object):
    ...
    def p_do(O, ssl, start):
        ...
        O.executable.append(ei_do(ssl=ssl, start=start,
                                  label=label,
                                  id_tok=tokenization.tk_identifier(
                                      ssl=ssl, i_code=i, value=code[i:j]),
                                  tokens=tokens))
    ...
    def p_if_elseif(O, ssl, keyword, start):
        ...
        if (code.startswith("then", i_clp+1) and len(code) == i_clp+5):
            ...
            if (keyword == "if"):
                ...
                ei = ei_elseif_then
            O.executable.append(
                ei(ssl=ssl, start=start, cond_tokens=cond_tokens))
            return None
        if (keyword != "if"):
            ssl.raise_syntax_error()
        i = i_clp + 1
        ...
        if (j < 0):
            if (start != 0):
                ssl.raise_syntax_error()
            O.executable.append(
                ei_if(ssl=ssl, start=start, cond_tokens=cond_tokens))
            return i
        labels = process_labels_list(
    ...
    def p_parameter(O, ssl, start):
        ...
        for tokens_l in tokens_ll:
            i_equal_signs = indices_of_tokenized_equal_signs(
                tokens=tokens_l.value)
        ...
        if (code.startswith("*", start+4)):
            if (code.startswith(",", start+5)):
                ...
                tz.collect_comma_separated_expressions(
                    ...
                    enable_implied_do=1)
                iolist = tokenization.remove_redundant_parentheses(
                    tokens=iolist)
                if (len(iolist) == 0):
                    ssl.raise_syntax_error(i=start+5)
...
class fproc(fproc_p_methods):
    ...
    def process_body_line(O, ssl, start):
        ...
        if (c == "("):
            ...
            c = code[i_clp+1]
            if (c == "="):
                O.process_assignment(ssl=ssl, start=start,
                                     i_equal_sign=i_clp+1)
                return
            if (c == "("):
                i_clp2 = ssl.index_of_closing_parenthesis(start=i_clp+2)
                if (i_clp2+1 < len(code) and code[i_clp2+1] == "="):
                    O.process_assignment(
                        ssl=ssl, start=start, i_equal_sign=i_clp2+1)
                    return
    ...
    def process_assignment(O, ssl, start, i_equal_sign):
        if (i_equal_sign+1 == len(ssl.code)):
            ssl.raise_syntax_error()
        lhs_tokens = tokenize_expression(
            ssl=ssl, start=start, stop=i_equal_sign)
    ...
    def build_fdecl_by_identifier(O):
        ...
        def get_implicit_data_type(id_tok, optional=False):
            ...
            if (result is None and not optional):
                id_tok.raise_semantic_error(
                    "Unknown data type: %s" % id_tok.value)
            return result
        ...
        for id_tok in O.args:
            ...
            if (tf is not None and tf.dim_tokens is not None):
                dim_id_toks = tokenization.extract_identifiers(
                    tokens=tf.dim_tokens)
            ...
            for ei in O.executable:
                if (ei.key == "goto"):
                    ...
                    if (ei.olist.err is not None):
                        tok = tokenization.get_statement_label_token(
                            tokens=ei.olist.err)
                        result.setdefault(tok.value, []).append(tok)
                elif (ei.key == "close"):
                    if (ei.cllist.err is not None):
                        tok = tokenization.get_statement_label_token(
                            tokens=ei.cllist.err)
                        result.setdefault(tok.value, []).append(tok)
                elif (ei.key == "inquire"):
                    if (ei.iuflist.err is not None):
                        tok = tokenization.get_statement_label_token(
                            tokens=ei.iuflist.err)
                        result.setdefault(tok.value, []).append(tok)
...
class split_fprocs(object):
    ...
    def process(O, stripped_source_lines):
        ...
        for curr_ssl in ssls:
            ...
            def collect_until_end(
                ...
                top_ssl.raise_error(msg="Missing END for %s" %
                                    (fproc_type.upper()))
    ...
    def show_counts_by_type(O, out=None, prefix=""):
        ...
        for attr in O.__slots__[:4]:
            print(prefix + "  %s: %s" %
                  (attr, len(getattr(O, attr))), file=out)
    ...
    def __init__(O, all_fprocs, top_procedures=None):
        ...
        for fproc in O.all_fprocs.all_in_input_order:
            for fdecl in fproc.fdecl_by_identifier.values():
                if (fdecl.is_user_defined_callable()
                    ...
                    primary_fproc = fprocs_by_name.get(
                        primary_external_identifier)
        ...
        def get_dependencies(fproc):
            ...
            for identifier in sorted(fproc.fdecl_by_identifier.keys()):
                if (identifier == fproc.name.value):
                    continue
            ...
            for top_procedure_or_procedures in O.top_procedures:
                for top_procedure in top_procedure_or_procedures.split(","):
                    ...
                    if (top_fproc is None):
                        top_fproc = fprocs_by_name.get(
                            "program_"+top_procedure)
                    if (top_fproc is None):
                        raise RuntimeError(
            ...
            if (fproc is not None):
                ...
                for dep in successors_by_node[identifier]:
                    if (dep in successors_by_node
                        ...
                        O.forward_uses_by_identifier.setdefault(
                            identifier, []).append(dep)
