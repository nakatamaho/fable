...
def check_intrinsics_extra(text):
    ...
    def check():
        if (len(lines) != 6):
            return False
        if (re.match(r'\d\d-[A-Z][a-z][a-z]-\d\d', lines[0]) is None):
            return False
        if (re.match(r'\d\d:\d\d:\d\d', lines[1]) is None):
            return False
        if (len(lines[2]) != 70):
            return False
        if (len(lines[3]) != 6):
        ...
        for i_ces, common_equivalence_simple in \
            ...
            try:
                O.comp_env.build(
                    ...
                    disable_warnings=(
                        file_name in file_names_disable_warnings),
            ...
            for info in io_infos:
                ...
                for exe_name in [fem_exe_name, ifort_exe_name]:
                        ...
                        if (opts.valgrind):
                            ...
                            def check(text):
                                ...
                                def assert_not_have_diffs():
                                    if (opts.keep_going):
                                        print(
                                            "WARNING: --keep-going after show_diff:", exe_name)
                                    else:
                    ...
                    if (file_name == "read_lines.f"):
                        exercise_end_of_line(
                            exe_name=exe_name, verbose=opts.verbose)
